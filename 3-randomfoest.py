...
train_data = pd.read_csv('train_adj_normalize_1216.csv',index_col=0)
train_feature=train_data.drop(['1day_label','5day_label','10day_label'],axis=1)
train_target=train_data['1day_label']

test_data = pd.read_csv('test_adj_normalize_1216.csv',index_col=0)
test_feature=test_data.drop(['1day_label','5day_label','10day_label'],axis=1)
test_target=test_data['1day_label']

model=RandomForestClassifier(max_features='sqrt',n_estimators=1,max_depth=6,min_samples_leaf=40,random_state=4)
